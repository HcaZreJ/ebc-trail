...
- **References**：`scripts/reportgen/citations.py` 渲染 `<ol class="refs">`，每条 `<li id="ref-NN">` 给标题、来源方、抓取日期、原始链接，带「引用于 §N」回链与折叠的要点摘录。被正文引用过零次的出处归到第二组「数据与方法来源」。
...
- **出处文件结构**：每份 `sources/NN-<主题>.md` 在一级标题与首个 `## 来源` 之间写一个 `## 要点` 小节，3–6 条 bullet，每条一句完整的话，承载一个数字、一个结论或一条待办。报告的 References 折叠块只渲染这一段，一手记录全文留在该段之后。标题行整行就是 `## 要点`——`## 对话要点（原文摘录，中文概括）` 这类不被识别为摘录段。
- **闸门**：标记引用的编号在 `sources/` 里没有对应文件、标记语法不合法（`[[7]]`、`[[abc]]` 这类展开后残留 `[[`）、或某份出处的 `## 要点` 缺失或为空，构建停下。
...
4. 这一节需要的出处写进 `sources/NN-<主题>.md`（含 `## 要点` 小节），正文事实用 `[[NN]]` 角标；需要的表格数字写进 `data/<名>.csv`。
...
2. 出处文件 `sources/NN-<主题>.md` 记 URL、抓取日期、提取出的数字，顶部写 `## 要点` 小节。
