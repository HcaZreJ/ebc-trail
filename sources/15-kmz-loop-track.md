...
- 轨迹来自用户自有的两步路 KMZ 文件，记录者 2024 年 5 月实走 Three Passes 大环线，`导航线` 里 20 条线首尾相接共 17,377 个带海拔的点，二维平面距离合计 183.6 km。
- 报告的每日里程与爬升由这条轨迹重算，处理分三步：按 25 m 步长沿轨迹重采样、用窗口 5 的滚动中位数平滑海拔、按 8 m 滞回阈值累计起伏。
- 平滑窗口取 5 的依据是与徒步文献口径对得最齐：文献给 Lukla–Phakding 约 200 m、Phakding–Namche 约 1,100 m，窗口 5 算出 206 m 与 1,109 m，窗口 3 与窗口 9 都偏得更远。
- 这个口径算出的爬升含同一天内的反复起伏，比攻略常引的约 6,000 m 高，因为攻略多按逐日净海拔差相加。
- 记录者翻 Kongma La 上山、经 Cho La 与 Gokyo 下山，轨迹不覆盖 Dingboche→Dughla→Lobuche 与 Lobuche/Pheriche 一带的标准走线，这几段用 Overpass 取的 OSM 步道几何加 OpenTopoData 的 SRTM 30 m 高程补上。
- 补测精度够用：SRTM30m 与 KMZ 实测 GPS 在 Lobuche 差 8 m、在 Kala Patthar 差 13 m，小于同一村庄内不同建筑的高差，两种数据放在同一张剖面图上不产生可见的接缝。
