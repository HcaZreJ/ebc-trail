...
- **`scripts/reportgen/` 的共用基础设施（`config.py` `csvio.py` `tables.py` `money.py` `imgio.py` `assemble.py`）由一个 agent 独占改。** 领域 provider（`figures.py` `costs.py` `quotes.py` `route.py` `appendix.py`）之间零 import 依赖，可以并行改。
