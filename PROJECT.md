...
- **`clothing-by-day.csv`**（人工整理，12 行数据，9 列）— 逐日穿衣搭配：当天最高点海拔、白天与夜间的气温区间、行进时上身穿哪几层、裤子穿哪条、日包里额外加带什么、这身搭配的推算依据（`why` 列，渲染成表里每天主行下面的通栏说明子行，缺失时构建停下）与出处。路段与宿营海拔不存在这张表里，由 `scripts/reportgen/clothing.py` 按 `day` 列从 `itinerary.csv` join 过来，行程一改穿衣表跟着变。`high_ele_m` 单独存是因为 `itinerary.csv` 记不了它——那张表的 `end_ele_m` 在 Day 6 适应日记的是宿营地 4,410m 而不是山脊 5,080m，Day 9 记的是终点 Pheriche 4,280m 而不是 Kala Patthar 5,545m；构建期校验 `high_ele_m` 不低于 `itinerary.csv` 里该天的任何一个海拔。`temp_day_c` 取当天行进路线跨过的海拔落差所对应的区间，`temp_night_c` 取宿营地的清晨最低温。
...
**待议事项二：是否走 Majestic Trails 直升机版套餐（USD 1,400/人、不含徒步餐食，2026-08-05 报价，见 sources/16）。** 本行程与他的日程结构一致（9.25 加都休整、只在 Dingboche 安排一个适应日），报告结论是可以签：直升机升级差价 USD 385 与市场差价约 USD 390 一致；按拼机实报下沿 USD 404/程 算他比自组贵每人约 USD 266（Lukla 会合档），向导背夫从加都随行、机票我们出时两边打平（差 USD 13）；按 2026 年挂牌 USD 650–700/程 算自组反超、他更便宜，且 12 人次旺季座位的凑座与天气改签由他兜。全包餐 USD 410 比自付约 USD 162 贵约 154%，不买。签约前等四件书面确认（1,400 按 6 人成团、含不含机场税；拼机还是包机、拆架与天气取消的处理；10.6 当天傍晚前回加都；向导背夫日薪口径与其固定翼交通安排）再定。这个决定与待议事项一联动：走套餐则向导背夫由他配。
