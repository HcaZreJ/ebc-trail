...
- 货币口径：所有花费换算展示为人民币（¥），换算汇率 **1 USD ≈ 6.8 CNY**、**1 CNY ≈ 19 NPR**（2026-07 参考价）。NPR 报价在表里保留原价，同时按 19 折出人民币。19 这个数取自 2025–2026 年多位徒步者的实付折算（NPR 399 折 ¥22、NPR 21,000 折 ¥1,100、NPR 500 折 ¥25），见 `sources/14`。
- 费用口径：`cost-breakdown.csv` 给**单点最佳估算**，不给区间。每行取值规则是先看有无标注日期的数据，有则取日期最接近 2026-09-24 的那个（同季同年的实付价优先于攻略站挂牌价），若可用数据的时间点都不接近就取各来源平均；取值理由写在该行 notes 里。
