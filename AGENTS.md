...
6. **人数分摊。** 共享成本按 `cost-breakdown.csv` 的 `shared_by_n` 列分摊：向导、向导背夫小费 ÷6；背夫、茶屋双人间、Thamel 双人间 ÷2；机场市区交通 ÷3；直升机包机参考行 ÷4（按 2026-07 询价一架坐 4 人）；其余 10 行按每人计。分摊语义只写在 `shared_by_n` 列里——`pp_usd` 恒等于「原报价 × `qty` ÷ `shared_by_n`」，双人间报每间价就把 ÷2 交给 `shared_by_n`，这条恒等式由 `tests/hidden/money_test.py` 逐行校验。同行人数 6 写在 `config.py` 的 `PAX`。
