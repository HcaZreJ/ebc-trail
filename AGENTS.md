...
3. **货币口径。** 花费以美元（USD）与尼泊尔卢比（NPR）计价，换算 1 USD ≈ 129.2 NPR（2026-07 参考价），写在 `scripts/reportgen/config.py` 的 `NPR_PER_USD`。单价按该项报出时的原始货币写进表格的「原报价」列（许可证写 NPR 3000、向导写 USD 32/天），可加总的每人金额列与合计统一用美元。129.2 由 1 USD ≈ 6.8 CNY 与 1 CNY ≈ 19 NPR 推出，19 取自 2025–2026 年多位徒步者的实付折算（NPR 399 折 ¥22、NPR 21,000 折 ¥1,100、NPR 500 折 ¥25），见 `sources/14-xiaohongshu-field-intel.md`。中国徒步者以人民币记录的一手实付价在 `sources/` 里保留原数字，同时给出美元折算。
